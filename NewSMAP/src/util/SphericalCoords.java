...
public class SphericalCoords {
	 ...
	 * from a starting lon lat.
	 */
	public static double latFor(double lonA, double latA, double distance,
			double azimuth) {
		return rad2Deg
				* Math.asin(Math.cos(azimuth * deg2Rad)
						* Math.sin(distance * deg2Rad) * Math.cos(latA * deg2Rad)
						+ Math.cos(distance * deg2Rad) * Math.sin(latA * deg2Rad));
	}
