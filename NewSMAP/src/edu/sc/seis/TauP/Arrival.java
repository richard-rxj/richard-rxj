...
public class Arrival implements Cloneable {
	...
	public Arrival(SeismicPhase phase, double time, double dist,
			double rayParam, int rayParamIndex, String name, String puristName,
			double sourceDepth) {
		this.phase = phase;
		this.time = time;
		this.dist = dist;
	...
	public String toString() {
		String desc = "time=" + time + " dist=" + dist + " rayParam="
				+ rayParam + " rayParamIndex=" + rayParamIndex + " name="
				+ name;
		if (pierce != null) {
			System.out.println("\nPierce:");
			for (int i = 0; i < pierce.length; i++) {
