cd D:\PhDWork\Jspace\Mobilesink\test\new\start5050\ImpactLocation\T100;
    ...
    C = load('unit-benefit-location-20.txt');
    N = C(:,1);
    E(:,1)=N;
    [m,n]=size(N);
    RB = C(:,2);
...
%     RB = C(:,2);
%     plot(N,RB,'-+g');
    C = load('unit-benefit-location-40.txt');
    RB = C(:,2);
    E(:,3)=RB;
    h=plot(N,RB,'-om');
    ...
    C = load('unit-benefit-location-60.txt');
    RB = C(:,2);
    E(:,4)=RB;
    h=plot(N,RB,'-pb');
    set(h,'MarkerSize',12);
...
%     plot(N,RB,'-pr');
    ...
    C = load('unit-benefit-location-80.txt');
    RB = C(:,2);
    E(:,5)=RB;
    h=plot(N,RB,'-sk');
    set(h,'MarkerSize',12);
...
%     plot(N,RB,'-sr');
    ...
    C = load('unit-benefit-location-100.txt');
    RB = C(:,2);
    E(:,6)=RB;
    h=plot(N,RB,'-^m');
    set(h,'MarkerSize',12);
...
%     C = load('unit-benefitgain-tour-T6400.txt');
%     RB = C(:,2);
%     E(:,7)=RB;
%     h=plot(N,RB,'-xm');
...
%       fid=fopen('D:\PhDWork\Jspace\Mobilesink\test\xmgracedata\location-throughput-unit.txt','w');%写入文件路径
