...
public class Utility {
	...
	/**
	 * Concatenate two bytes to a short integer value. Accepts a high and low
	 * byte to be converted to a 16-bit integer. if swapBytes is true, then
	 * <b>b</b> becomes the high order byte.
	 * 
	 * @param a
	 *            high order byte
	 * @param b
	 ...
	 * 
	 * @param a
	 *            high order unsigned byte
	 * @param b
	 *            low order unsigned byte
	 * @param swapBytes
	 *            if true, <b>b</b> becomes the high order byte
	 * @return positive 32-bit integer
	 */
	...
	/**
	 * Conacatenate four unsigned byte values into a long integer. This method
	 * puts out a long value because a large unsigned 32-bit value would exceed
	 * the capacity of an int, which is considered signed in Java.
	 * 
	 * @param a
	 *            highest-order byte
	 * @param b
	 ...
	 *            lowest order byte
	 * @param swapBytes
	 *            if true, byte order is <b>d,c,b,a</b>, else order is
	 *            <b>a,b,c,d</b>
	 * @return positive long integer
	 */
	public static long uBytesToLong(byte a, byte b, byte c, byte d,
			boolean swapBytes) {
		if (swapBytes) {
			return ((a & 0xffL)) + ((b & 0xffL) << 8) + ((c & 0xffL) << 16)
					+ ((d & 0xffL) << 24);
		} else {
			return ((a & 0xffL) << 24) + ((b & 0xffL) << 16)
					+ ((c & 0xffL) << 8) + ((d & 0xffL));
		}
	}
	 ...
	 * contents of <b>source</b> and is padded on the end with
	 * <b>paddingByte</b>. If <b>requiredBytes</b> is less than or equal to the
	 * length of <b>source</b>, then <b>source</b> will simply be returned.
	 * 
	 * @param source
	 *            byte array to have <b>paddingByte</b>(s) appended to
	 * @param requiredBytes
	 *            the length in bytes of the returned byte array
	 * @param paddingByte
	 *            the byte value that will be appended to the array to fill out
	 *            the required byte size of the return array
	...
	public static String getSequenceNumber(BinaryRandomInput input) {
		...
		return (new String(sequenceNumber));
	}

	public static String getDataHeader(BinaryRandomInput input) {
