...
%     RB = C(:,2);
%     plot(N,RB,'-+g');
    C = load('disTra-benefitgain-tour-T200.txt');
    RB = C(:,2);
    E(:,3)=RB;
    h=plot(N,RB,'-om');
    ...
    C = load('disTra-benefitgain-tour-T400.txt');
    RB = C(:,2);
    E(:,4)=RB;
    h=plot(N,RB,'-pb');
    set(h,'MarkerSize',12);
...
%     plot(N,RB,'-pr');
    ...
    C = load('disTra-benefitgain-tour-T800.txt');
    RB = C(:,2);
    E(:,5)=RB;
    h=plot(N,RB,'-sk');
    set(h,'MarkerSize',12);
...
%     plot(N,RB,'-sr');
    ...
    C = load('disTra-benefitgain-tour-T3200.txt');
    RB = C(:,2);
    E(:,6)=RB;
    h=plot(N,RB,'-^m');
    set(h,'MarkerSize',12);
...
%     legend('\lambda =0','\lambda =0.05','\lambda =0.1','\lambda =0.2','\lambda =0.3','\lambda =0.4','\lambda =0.5','\lambda =0.6','\lambda =0.7','\lambda =0.8','\lambda =0.9','\lambda =1',2);
   ...
   % set(gcf, 'PaperPosition', [2 1 4 2]);
    ...
    %saveas(gcf,v,'eps');
      ...
      fid=fopen('D:\PhDWork\Jspace\Mobilesink\test\xmgracedata\time-throughput-disTra.txt','w');%写入文件路径
