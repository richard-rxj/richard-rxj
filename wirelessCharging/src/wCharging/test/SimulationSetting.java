...
public class SimulationSetting {
	...
	public static double stepWaitingConstant = 1; // if no feasible then keep
													// still and wait
	...
	static {
		...
		try {
			FileHandler htmlHandler = new FileHandler(
					SimulationSetting.class.getName() + ".html");
	...
	public static ChargingRequestQueue generateRequest(int totalRequest) {
		...
		for (int i = 1; i <= totalRequest; i++) {
			...
			c.setResidualLifetime(Math.random() * 0.5
					* SimulationSetting.timeLimit);
			resultQueue.add(c);
