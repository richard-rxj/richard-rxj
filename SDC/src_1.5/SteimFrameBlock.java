...
/**
 * This class acts as a container to hold encoded bytes processed by a Steim
 * compression routine, as well as supporting information relating to the data
 * processed. It also facilitates Steim notation and the formation of the data
 * frames. This class stores the Steim encoding, but is ignorant of the encoding
...
public class SteimFrameBlock {
	 ...
	 * copression. Instantiate object with the number of 64-byte frames that
	 * this block will contain (should connect to data record header such that a
	 * proper power of 2 boundary is formed for the data record) AND the version
	 * of Steim compression used (1 and 2 currently) the number of frames
	 * remains static...frames that are not filled with data are simply full of
	 * nulls.
	 * 
	 * @param numFrames
	 *            the number of frames in this Steim record
	 * @param steimVersion
	...
	public SteimFrameBlock(int numFrames, int steimVersion) {
		...
		this.steimFrame[currentFrame].pos++; // increment position in frame to
												// W1
	}
	 ...
	 * 
	 * @param samples
	 *            the number of sample differences in the word
	 * @param nibble
	 *            a value of 0 to 3 that reflects the W0 encoding for this word
	 * @return boolean indicating true if the block is full (ie: the calling app
	 *         should not add any more to this object)
	 */
	...
	/**
	 * Set the reverse integration constant X(N) explicitly to the provided word
	 * value. This method is typically used to reset X(N) should the compressor
	 ...
	 * 
	 * @param bitFlag
	 *            a value 0 to 3 representing an encoding nibble
	 */
	private void addEncodingNibble(int bitFlag) {
		int offset = steimFrame[currentFrame].pos; // W0 nibble offset -
													// determines Cn in W0
	...
	/**
	 * This represents a single Steim compression frame. It stores values as an
	 * int array and keeps track of it's current position in the frame.
	 */
	private class SteimFrame {
		public int[] word = new int[16]; // 16 32-byte words
		public int pos = 0; // word position in frame (pos: 0 = W0, 1 = W1,
							// etc...)
	}
