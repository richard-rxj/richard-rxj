 ...
 *
 */
public class PathBasedCentralizedSolution extends Solution {
	...
	public Coverage schedule() {
		...
		ExperimentSetting.gLog.info(String.format("*****PathBased-%s-network<%d>-target<%d> begin", this.func.getClass().getName(), this.network.getSensors().size(), this.network.getTargets().size()));
		...
		while(true) {
			...
			loop++;
			ExperimentSetting.gLog.severe(String.format("PathBased-Iteration<%d>----begin", loop));
