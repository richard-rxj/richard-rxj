cd D:\PhDWork\Jspace\Mobilesink\test\new\ImpactWeight50;
    ...
    C = load('unit-benefit-weight-11.txt');
    N = C(:,1);
    [m,n]=size(N);
    RB = C(:,2);
    %subplot(1,2,1);
...
%     RB = C(:,2);
%     plot(N,RB,'-+g');
    C = load('unit-utility-weight-20.txt');
    RB = C(:,2);
    h=plot(N,RB,'-om');
    ...
    C = load('unit-utility-weight-80.txt');
    RB = C(:,2);
    h=plot(N,RB,'-pb');
    set(h,'MarkerSize',12);
...
%     plot(N,RB,'-sr');
    ...
    C = load('unit-utility-weight-1000.txt');
    RB = C(:,2);
    h=plot(N,RB,'-^m');
    set(h,'MarkerSize',12);
...
%     C = load('unit-utility-weight-10000.txt');
%     RB = C(:,2);
%     h=plot(N,RB,'-xm');
...
%     plot(N,RB,'-xr');
    ...
    legend('Max\_Gain','Max\_UtilityGain (a = 2)','Max\_UtilityGain (a = 8)','Max\_UtilityGain (a = 32)','Max\_UtilityGain (a = 100)',2);
