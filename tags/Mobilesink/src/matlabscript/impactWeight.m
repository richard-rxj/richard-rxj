cd D:\PhDWork\Jspace\Mobilesink\test\ImpactWeight-rg-5;

    %v = strcat(strcat('running_',int2str(i)),'.txt');
    Size=[100,200,300,400,500,600];
    C = load('linear-weight-0.txt');
    N = C(:,1);
    [m,n]=size(N);
    RB = C(:,2);
    %subplot(1,2,1);
...
%     plot(N,RB,'-xr');
    ...
    legend('\lambda =0','\lambda =0.2','\lambda =0.4','\lambda =0.6','\lambda =0.8','\lambda =1',2);
